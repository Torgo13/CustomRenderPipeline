...
namespace UnityEngine.Rendering.HighDefinition
{
    ...
    /// </summary>
    [Serializable, VolumeComponentMenuForRenderPipeline("Fog", typeof(UniversalRenderPipeline))]
    ...
    public class Fog : VolumeComponent
    {
        ...
        {
            return /*hdCamera.frameSettings.IsEnabled(FrameSettingsField.AtmosphericScattering) &&*/ hdCamera.camera.GetUniversalAdditionalCameraData().volumeStack.GetComponent<Fog>().enabled.value;
        }

        internal static bool IsVolumetricFogEnabled(HDCamera hdCamera)
        {
            var fog = hdCamera.camera.GetUniversalAdditionalCameraData().volumeStack.GetComponent<Fog>();

            bool a = fog.enableVolumetricFog.value;
            bool b = true; //hdCamera.frameSettings.IsEnabled(FrameSettingsField.Volumetrics);
        ...
        {
            //var visualEnv = hdCamera.camera.GetUniversalAdditionalCameraData().volumeStack.GetComponent<VisualEnvironment>();
        ...
        internal static void UpdateShaderVariablesGlobalCB(ref ShaderVariablesGlobal cb, HDCamera hdCamera)
        {
            ...
            var fogSettings = hdCamera.camera.GetUniversalAdditionalCameraData().volumeStack.GetComponent<Fog>();
